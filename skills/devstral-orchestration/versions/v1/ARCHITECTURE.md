...
> Estado al **2026-06-21**. Documenta la configuración completa de Claude Code
> CLI orquestando modelos locales. Fuente de verdad de qué corre dónde y por qué.

## Visión general — orquestación en 4 niveles (cascada de aislamiento de contexto)

```
Opus (loop principal, vos) ── ARQUITECTO ────────────────────────────────┐
  planes · arquitectura · seguridad crítica · review final · routing      │
  │                                                                        │
  ├─ Agent(model:sonnet) ──► subagentes Sonnet ── OBRERO                   │
...
**Principio**: el costo dominante es el contexto que acumula el orquestador (Opus).
Delegar usa un modelo más barato **y** descarta el contexto de la sub-tarea (solo
vuelve el resumen). Regla madre: Opus lee poco, delega mucho, recibe resúmenes.
Al local solo va lo mecánico + verificable + inequívoco. Routing completo en el
skill `devstral-orchestration`.
